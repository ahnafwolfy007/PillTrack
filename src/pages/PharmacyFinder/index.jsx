...
import { 
    MapPin, Search, Navigation, Phone, Star, Package, 
    Loader2, X, ChevronRight, AlertCircle, CheckCircle,
    Clock, Building2, ArrowRight, Crosshair
} from 'lucide-react';
...
const PharmacyFinder = () => {
    ...
    useEffect(() => {
        ...
        if (medicineName.length >= 2) {
            suggestionsTimeoutRef.current = setTimeout(async () => {
                try {
                    const results = await pharmacyFinderService.getMedicineSuggestions(medicineName);
    ...
    const getCurrentLocation = useCallback(() => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (err) => {
                ...
                setError('Could not get your location. Please click on the map to set your location.');
                setIsLoadingLocation(false);
            },
    ...
    const handleSearch = async () => {
        if (!userLocation) {
            setError('Please select your location first by clicking on the map or using the "Use My Location" button');
            return;
        }
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <Button
                                    ...
                                    disabled={isSearching || !userLocation || !medicineName.trim()}
                                    className="w-full gap-2"
                                >
                                    {isSearching ? (
                                        <Loader2 className="w-4 h-4 animate-spin" />
                                ...
                                <CardContent>
                                    {searchResults.allResults?.length === 0 ? (
                                        <div className="text-center py-6 text-gray-500">
                                            ...
                                            <p>No pharmacies found with "{searchResults.searchedMedicine}"</p>
                                ...
                                >
                                    ...
                                    {searchResults?.allResults?.map((pharmacy, index) => (
                                        <Marker
                                            ...
                                            icon={index === 0 ? nearestPharmacyIcon : 
                                                  selectedPharmacy?.pharmacyId === pharmacy.pharmacyId ? selectedPharmacyIcon : 
                                                  pharmacyIcon}
                                            eventHandlers={{
                                        ...
                                        >
                                            <Popup>
                                                <div className="min-w-[200px]">
                                                    ...
                                                    <h3 className="font-bold text-lg">{pharmacy.pharmacyName}</h3>
                                                    ...
                                                    <div className="mt-2 space-y-1">
                                                        ...
                                                        <p className="text-sm">
                                                            <strong>Price:</strong> ৳{pharmacy.price?.toFixed(2)}
                                                        </p>
                                                        <p className="text-sm">
                                                            <strong>Distance:</strong> {pharmacy.distanceFormatted}
                                                        </p>
                                                        <p className="text-sm">
                                                            <strong>Stock:</strong> {pharmacy.stockQuantity} units
                                                        </p>
                                                    </div>
                                                </div>
                                            </Popup>
                                    ...
                                    {!searchResults && allPharmacies.map((pharmacy) => (
                                        <Marker
                                            key={pharmacy.id}
                                            position={[pharmacy.latitude, pharmacy.longitude]}
                                            icon={pharmacyIcon}
                                        >
                                            <Popup>
                                                <div className="min-w-[180px]">
                                                    ...
                                                    <p className="text-sm text-gray-600">{pharmacy.area}</p>
                                                    <div className="flex items-center gap-1 mt-1">
                                                        <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" />
                                                        <span className="text-sm">{pharmacy.rating?.toFixed(1)}</span>
                                                        <span className="text-xs text-gray-400">({pharmacy.ratingCount})</span>
                                                    </div>
                                    ...
                                    <CardContent className="p-6">
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
                                            <div className="bg-blue-50 p-4 rounded-lg">
                                                <div className="text-sm text-blue-600 font-medium">Medicine</div>
                                                <div className="text-lg font-bold text-gray-800 truncate">
                                                    {selectedPharmacy.medicineName}
                                                </div>
                                            </div>
                                            <div className="bg-green-50 p-4 rounded-lg">
                                                <div className="text-sm text-green-600 font-medium">Price</div>
                                                <div className="text-lg font-bold text-gray-800">
                                                    ৳{selectedPharmacy.price?.toFixed(2)}
                                                </div>
                                            </div>
                                            <div className="bg-purple-50 p-4 rounded-lg">
                                                <div className="text-sm text-purple-600 font-medium">In Stock</div>
                                                <div className="text-lg font-bold text-gray-800">
                                                    {selectedPharmacy.stockQuantity} units
                                                </div>
                                            </div>
                                            <div className="bg-orange-50 p-4 rounded-lg">
                                                <div className="text-sm text-orange-600 font-medium">Form</div>
                                                <div className="text-lg font-bold text-gray-800 truncate">
                ...
                <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Card>
                        <CardContent className="p-4 text-center">
                            <div className="text-3xl font-bold text-primary">{allPharmacies.length}</div>
