    ...
    try {
      ...
      if (shopMedicineData) {
        const productData = {
          ...
          name:
            shopMedicineData.medicineName ||
            shopMedicineData.medicine?.brandName ||
            "Medicine",
          genericName: shopMedicineData.medicine?.genericName || "",
          brand: shopMedicineData.medicine?.manufacturer || "Brand",
          type:
            shopMedicineData.categoryName ||
            shopMedicineData.medicine?.categoryName ||
            "General",
          rating: 4.5,
          reviews: Math.floor(Math.random() * 200) + 50,
          image:
            shopMedicineData.imageUrl ||
            shopMedicineData.medicine?.imageUrl ||
            "https://pngimg.com/uploads/pill/pill_PNG17239.png",
          description:
            shopMedicineData.medicine?.description ||
            shopMedicineData.description ||
            "No description available.",
          dosage:
            shopMedicineData.medicine?.dosageForm || "Follow prescribed dosage",
          sideEffects: shopMedicineData.medicine?.sideEffects?.split(",") || [
            "Consult your doctor",
          ],
          warnings: shopMedicineData.medicine?.warnings?.split(",") || [
        ...
        setShops([
          {
            ...
            name:
              shopMedicineData.shopName ||
              shopMedicineData.shop?.name ||
              "Pharmacy",
        ...
        if (shopMedicineData.medicineId) {
          try {
            const otherShops = await shopMedicineService.getShopsSelling(
              shopMedicineData.medicineId
            );
            if (
              otherShops &&
              Array.isArray(otherShops) &&
              otherShops.length > 0
            ) {
              setShops(
                otherShops.map((s) => ({
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          >
            <Card className="border-none shadow-md overflow-hidden">
              <CardContent className="p-8">
                <div className="relative aspect-square bg-gradient-to-br from-blue-50 to-slate-50 dark:from-slate-700 dark:to-slate-800 rounded-2xl flex items-center justify-center">
                  <motion.img
                    src={product.image}
                    alt={product.name}
          ...
          >
            ...
            {/* Shop Selection */}
            <Card className="border-none shadow-sm bg-slate-50 dark:bg-slate-800">
              <CardContent className="p-4">
                <h3 className="font-semibold text-slate-800 dark:text-white mb-3">
                  Available from {shops.length} shops
                </h3>
                  ...
                  {shops.map((shop, i) => (
                    <div
                      ...
                      className={cn(
                        ...
                        selectedShop === i
                          ? "bg-white dark:bg-slate-700 border-2 border-primary shadow-sm"
                          : "bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:border-slate-300 dark:hover:border-slate-500"
                      )}
                    >
            ...
            <div className="flex items-center gap-4">
              ...
              >
                ...
                Add to Cart - $
                {(parseFloat(shops[selectedShop].price) * quantity).toFixed(2)}
              </Button>
            </div>
            ...
            {relatedProducts.map((p) => (
              <Link key={p.id} to={`/marketplace/${p.id}`}>
                <Card className="border-none shadow-sm hover:shadow-md transition-all group cursor-pointer">
                  <CardContent className="p-4">
                    <div className="aspect-square bg-slate-50 dark:bg-slate-700 rounded-xl flex items-center justify-center mb-4 group-hover:bg-blue-50 dark:group-hover:bg-blue-900/20 transition-colors">
                      <img
                        src={p.image}
                        alt={p.name}
