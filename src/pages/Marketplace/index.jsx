...
import { 
    Search, ShoppingCart, Plus, Star, MapPin, Heart, Loader2, 
    Navigation, Phone, Clock, X, ChevronRight, Building2, Package,
    TrendingUp, Crosshair, ArrowRight, AlertCircle
} from 'lucide-react';
...
import { shopMedicineService, categoryService, medicineService, pharmacyFinderService } from '../../services/api';
...
const MedicineCard = ({ id, name, brand, price, rating, type, image, onAddToCart, onFindNearby }) => {
...
const PharmacyResultCard = ({ pharmacy, isNearest, onNavigate, onAddToCart }) => (
    <motion.div
    ...
    >
        <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
                <div className={cn(
                    ...
                    isNearest ? "bg-amber-400 text-white" : "bg-primary/10 text-primary"
                )}>
                    <Building2 size={24} />
                </div>
                <div>
        ...
        <div className="flex items-center justify-between text-sm mb-3">
            ...
            <span className="font-bold text-lg text-primary">৳{pharmacy.price?.toFixed(2)}</span>
        </div>
...
const Marketplace = () => {
    ...
    useEffect(() => {
        ...
        if (searchQuery.length >= 2) {
            suggestionsTimeoutRef.current = setTimeout(async () => {
                try {
                    const results = await pharmacyFinderService.getMedicineSuggestions(searchQuery);
    ...
    const fetchData = async () => {
        try {
            ...
            const mappedPopular = (popularData?.content || []).map(item => ({
                ...
                brand: item.manufacturerName || item.manufacturer?.name || 'Manufacturer',
                price: (item.unitPrice || Math.random() * 500 + 50).toFixed(2),
                rating: 4 + Math.random(),
                type: item.type || item.categoryName || 'General',
                image: item.imageUrl || 'https://pngimg.com/uploads/pill/pill_PNG17239.png',
    ...
    const searchNearbyPharmacies = async (medicineName) => {
        ...
        try {
            ...
            setNearbyResults({ pharmaciesWithMedicine: [], totalFound: 0, allResults: [] });
        } finally {
            ...
            <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-slate-100">
                <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                    <Link to="/" className="font-bold text-xl text-slate-800 flex items-center gap-2">
                        ...
                        <span className="bg-clip-text text-transparent bg-gradient-to-r from-primary to-blue-600">PillTrack Market</span>
                    </Link>
                    <form onSubmit={handleSearchSubmit} className="hidden md:flex flex-1 max-w-xl mx-8 relative items-center">
                        ...
                        <div className="container mx-auto px-4 py-6">
                            ...
                            {isSearchingNearby ? (
                                ...
                                <div className="grid lg:grid-cols-2 gap-6">
                                    ...
                                    <div className="h-[400px] rounded-xl overflow-hidden shadow-lg border border-slate-200">
                                        ...
                                        >
                                            ...
                                            {/* Pharmacy markers */}
                                            {nearbyResults?.pharmaciesWithMedicine?.map((pharmacy, index) => (
                                                <Marker
                                                    key={pharmacy.shopMedicineId || index}
                                                    position={[pharmacy.latitude, pharmacy.longitude]}
                                                    icon={index === 0 ? nearestPharmacyIcon : pharmacyIcon}
                                                >
                                                    <Popup>
                                                        <div className="min-w-[200px]">
                                                            <h4 className="font-bold text-lg mb-1">{pharmacy.shopName}</h4>
                                                            <p className="text-sm text-slate-600 mb-2">{pharmacy.area}</p>
                                                            <div className="flex items-center justify-between text-sm">
                                                                <span className="text-primary font-bold">৳{pharmacy.price?.toFixed(2)}</span>
                                                                <span className="text-slate-500">{pharmacy.distance?.toFixed(2)} km</span>
                                                            </div>
                                    ...
                                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2">
                                        {nearbyResults?.pharmaciesWithMedicine?.slice(0, 10).map((pharmacy, index) => (
                                            <PharmacyResultCard
                                                key={pharmacy.shopMedicineId || index}
                                                pharmacy={pharmacy}
                                                isNearest={index === 0}
                                ...
                                <div className="text-center py-8">
                                    ...
                                    <p className="text-slate-600">No pharmacies found with this medicine within 15km.</p>
                                    <p className="text-sm text-slate-500 mt-1">Try a different medicine or expand your search area.</p>
                                </div>
                            )}
                        </div>
                    </motion.div>
                    ...
                    <section className="mb-12">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-slate-600 text-sm mt-1">Most searched and purchased medicines</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4">
                            {popularMedicines.slice(0, 6).map((medicine) => (
                                ...
                                >
                                    ...
                                    <h4 className="font-semibold text-slate-800 line-clamp-1 text-sm">{medicine.name}</h4>
                                    <p className="text-xs text-slate-500 line-clamp-1">{medicine.genericName || medicine.brand}</p>
