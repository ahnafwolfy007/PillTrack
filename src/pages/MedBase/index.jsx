                        ...
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                <div className="bg-blue-50 rounded-lg p-4">
                                    ...
                                    <p className="font-medium text-blue-800 capitalize">{medicine.type || 'N/A'}</p>
                                </div>
                                <div className="bg-green-50 rounded-lg p-4">
                                    <p className="text-xs text-green-600 mb-1">Container</p>
                                    <p className="font-medium text-green-800">{medicine.containerType || 'N/A'}</p>
                                </div>
                                <div className="bg-purple-50 rounded-lg p-4">
                                    <p className="text-xs text-purple-600 mb-1">Unit Quantity</p>
                                    <p className="font-medium text-purple-800">{medicine.unitQuantity || 'N/A'}</p>
                                </div>
                            </div>
                        ...
                        <div className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border border-green-200">
                                    ...
                                    <p className="text-3xl font-bold text-green-800">{formatPrice(medicine.unitPrice)}</p>
                                ...
                                <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                                    ...
                                    <p className="text-3xl font-bold text-blue-800">{medicine.packQuantity || 'N/A'}</p>
                                ...
                                <div className="bg-gradient-to-br from-purple-50 to-violet-50 rounded-xl p-6 border border-purple-200">
                                    ...
                                    <p className="text-3xl font-bold text-purple-800">{formatPrice(medicine.packPrice)}</p>
                                    <p className="text-xs text-purple-600 mt-1">Full pack</p>
                                </div>
                            </div>
                            
                            <div className="bg-slate-50 rounded-lg p-4">
                                <h4 className="font-medium text-slate-800 mb-2">Manufacturer</h4>
                                <p className="text-slate-600">{medicine.manufacturerName || 'Not specified'}</p>
                            </div>
                        </div>
                    )}
                    ...
                    {activeTab === 'alternatives' && (
                        <div className="space-y-4">
                            <p className="text-slate-600 text-sm">
                                Medicines with the same generic component: <strong>{medicine.genericName}</strong>
                            </p>
                            ...
                            {loadingAlternatives ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {alternatives.map(alt => (
                                        <div key={alt.id} className="bg-slate-50 rounded-lg p-4 hover:bg-slate-100 transition-colors">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    <h4 className="font-medium text-slate-800">{alt.brandName}</h4>
                                                    <p className="text-sm text-slate-500">{alt.strength} • {alt.dosageForm}</p>
                                                    <p className="text-xs text-slate-400 mt-1">{alt.manufacturerName}</p>
                                                </div>
                                                <div className="text-right">
                                                    <p className="font-bold text-primary">{formatPrice(alt.unitPrice)}</p>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
    ...
    const fetchInitialData = async () => {
        try {
            ...
            if (Array.isArray(typesData) && typesData.length > 0) {
                ...
                const formattedTypes = typesData.map(t => 
    ...
    const loadMoreMedicines = async () => {
        try {
            ...
            if (searchQuery.length >= 2) {
                ...
                data = await medicineService.getByType(selectedType.toLowerCase(), nextPage, 24);
            } else {
            ...
            <section className="container mx-auto px-4 py-12 text-center">
                ...
                >
                    <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">
                        Medicine <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-blue-600">Database</span>
                    </h1>
                    <p className="text-lg text-slate-600 max-w-2xl mx-auto mb-8">
                        Search our comprehensive database of {totalElements.toLocaleString()}+ medicines. 
                        Find information about brands, generics, prices, and alternatives.
                    </p>
            ...
            <section className="container mx-auto px-4 pb-12">
                <div className="mb-4 flex items-center justify-between">
                    <span className="text-slate-500 text-sm">
                        {medicines.length} medicine{medicines.length !== 1 ? 's' : ''} displayed
                        {totalElements > medicines.length && ` of ${totalElements.toLocaleString()}`}
                    </span>
                ...
                {medicines.length === 0 ? (
                            ...
                            >
                                ...
                                >
                                    <div className="h-24 bg-gradient-to-br from-slate-50 to-blue-50 p-4 flex items-center justify-center group-hover:from-blue-50 group-hover:to-primary/10 transition-colors relative">
                                        <Pill size={36} className="text-primary/30 group-hover:scale-110 transition-transform" />
                                        {medicine.type && (
                                    ...
                                    <CardContent className="p-4">
                                        ...
                                        <p className="text-xs text-slate-500 mb-1 line-clamp-1">{medicine.genericName}</p>
                                        <p className="text-sm text-slate-600 line-clamp-1">{medicine.strength}</p>
                                        ...
                                        <div className="mt-3 pt-3 border-t border-slate-100">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-1 text-xs text-slate-400 line-clamp-1 flex-1">
                                                    ...
                                                    <span className="truncate">{medicine.manufacturerName || 'Unknown'}</span>
                                                </div>
                                            </div>
                    ...
                    {page < totalPages - 1 && (
                        <div className="text-center mt-8">
                            <Button variant="outline" onClick={loadMoreMedicines} className="px-8">
                                Load More Medicines
                            </Button>
                        </div>
                    )}
