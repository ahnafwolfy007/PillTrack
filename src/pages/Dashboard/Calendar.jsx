...
const Calendar = () => {
  ...
  const fetchCalendarData = async () => {
    ...
    try {
      ...
      // Add dose log events
      const logs = logsResponse.success
      ...
      const meds = medsResponse.success
        ? Array.isArray(medsResponse.data)
          ? medsResponse.data
          : []
        : [];
      const { hours: currentHour, minutes: currentMin } =
        getCurrentTimeInDhaka();
  ...
  const handleMarkTaken = async (event) => {
    try {
      if (event.logId) {
        ...
        const scheduledTime =
          event.time instanceof Date ? event.time.toISOString() : event.time;
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            View and manage your medication calendar.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="space-y-6">
                  ...
                  {(selectedEvent.status === "upcoming" ||
                    ...
                    <div className="flex gap-2 pt-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className="flex-1 gap-1"
                        onClick={() => handleMarkSkipped(selectedEvent)}
                      >
                        <X size={14} /> Skip
                      </Button>
                    </div>
          ...
          {/* Adherence Stats */}
          <Card className="border-none shadow-md bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/30 dark:to-emerald-900/30">
          ...
          {/* Timezone Info */}
          <Card className="border-none shadow-sm bg-blue-50 dark:bg-blue-900/30">
            <CardContent className="p-4 flex items-center gap-2 text-sm text-blue-700 dark:text-blue-300">
              <MapPin size={16} />
