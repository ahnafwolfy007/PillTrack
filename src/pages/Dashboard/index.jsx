...
import { Pill, Clock, AlertCircle, CheckCircle, TrendingUp, Calendar as CalendarIcon, MoreVertical, Plus, ShoppingCart, Loader2, Volume2, MapPin, Package, SkipForward, X, RefreshCw } from 'lucide-react';
...
const Dashboard = () => {
    ...
    const fetchDashboardData = async () => {
        try {
            ...
            const medsData = medsResponse.success ? medsResponse.data : (Array.isArray(medsResponse) ? medsResponse : []);
            ...
            const activeMedsArr = (medsData || []).filter(m => m.status === 'ACTIVE' || m.isActive === true);
            ...
            const adherenceScore = completedDoses > 0 ? Math.round((takenDoses / completedDoses) * 100) : 100;
            ...
            const refillData = (medsData || [])
                ...
                .map(med => {
                    ...
                    const daysUntilEmpty = dailyDoses > 0 ? Math.floor(currentQty / dailyDoses) : 999;
            ...
            setStats({
                adherenceScore,
                activeMeds,
                nextRefillDays: refillData.length > 0 ? `${refillData[0].daysLeft} days` : 'N/A',
    ...
    const upcomingDose = sortedPendingDoses.find(d => d.totalMinutes >= currentTotalMinutes) || sortedPendingDoses[0];
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                {[
                    { label: 'Adherence', value: `${stats.adherenceScore}%`, icon: TrendingUp, color: 'text-green-500', bg: 'bg-green-50' },
                    { label: 'Active Meds', value: stats.activeMeds.toString(), icon: Pill, color: 'text-blue-500', bg: 'bg-blue-50' },
                    { label: 'Taken', value: doseStats.taken.toString(), icon: CheckCircle, color: 'text-emerald-500', bg: 'bg-emerald-50' },
                    { label: 'Skipped', value: doseStats.skipped.toString(), icon: SkipForward, color: 'text-amber-500', bg: 'bg-amber-50' },
                    { label: 'Missed', value: doseStats.missed.toString(), icon: X, color: 'text-red-500', bg: 'bg-red-50' },
                    { label: 'Pending', value: doseStats.pending.toString(), icon: Clock, color: 'text-blue-500', bg: 'bg-blue-50' },
                ].map((stat, i) => (
                    <motion.div
                        key={i}
                                ...
                                {upcomingDose ? (
                                    <div className="space-y-6 text-center sm:text-left">
                                        ...
                                        <div className="flex flex-wrap gap-3 justify-center sm:justify-start">
                                            <Button size="lg" className="bg-white text-primary hover:bg-white/90 shadow-xl border-0" onClick={() => handleMarkAsTaken(upcomingDose)}>
                                                Take Now
                                            </Button>
                ...
                <div className="space-y-6">
                    ...
                    <Card className="border-none shadow-md">
                        <CardHeader className="pb-2">
                            ...
                            <CardDescription>{doseStats.total} doses scheduled</CardDescription>
                        </CardHeader>
                        <CardContent className="h-[200px] relative">
                            ...
                            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                                <span className="text-2xl font-bold text-slate-800">{stats.adherenceScore}%</span>
                                <span className="text-xs text-slate-500">Adherence</span>
                            </div>
                        </CardContent>
                        <div className="px-4 pb-4 grid grid-cols-2 gap-2 text-xs">
                            {adherenceData.filter(d => d.name !== 'No Data').map((d, i) => (
                                ...
                                refillInfo.slice(0, 4).map((med, i) => (
                                    <div 
                                        ...
                                        className={cn(
                                            ...
                                            med.needsRefillSoon ? "bg-amber-50 border-amber-200" : "bg-slate-50 border-slate-100"
                                        )}
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <h5 className="font-semibold text-slate-800 text-sm">{med.name}</h5>
                                            <span className={cn(
                                                "text-xs font-bold px-2 py-0.5 rounded-full",
                                                med.daysLeft <= 3 ? "bg-red-100 text-red-600" :
                                                med.daysLeft <= 7 ? "bg-amber-100 text-amber-600" :
                                        ...
                                        <div className="mt-2 h-1.5 bg-slate-200 rounded-full overflow-hidden">
                                            <div 
                                                className={cn(
                                                    ...
                                                    med.daysLeft <= 3 ? "bg-red-500" :
                                                    med.daysLeft <= 7 ? "bg-amber-500" :
                                                    "bg-green-500"
                                                )}
                                                style={{ width: `${Math.min(100, (med.daysLeft / 30) * 100)}%` }}
                                            />
                                        </div>
                                    </div>
                                ))
                        ...
                        <CardContent className="p-4 flex gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-sm text-slate-600 mt-1">
                                    ...
                                    {lowStockMeds[0]?.name} has only {lowStockMeds[0]?.inventory || 0} pills left.
                                </p>
