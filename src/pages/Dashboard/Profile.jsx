...
const Profile = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      if (medsResponse.success && medsResponse.data) {
        const meds = Array.isArray(medsResponse.data)
          ? medsResponse.data
          : medsResponse.data.content || [];
        ...
        const daysTracking = firstMed
          ? Math.floor((Date.now() - firstMed) / (1000 * 60 * 60 * 24))
          : 0;
        ...
        setStats([
          ...
          {
            label: "Days Tracking",
            value: Math.max(1, daysTracking).toString(),
            icon: Calendar,
            color: "text-purple-600 bg-purple-50",
          },
        ]);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {success && (
        <div className="p-4 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-lg text-green-700 dark:text-green-400"></div>
      )}
            ...
            <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                {isEditing && canEditBloodType ? (
                  <select
                    className="flex h-10 w-full rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 text-sm dark:text-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50"
                  ...
                  <div className="flex h-10 w-full items-center rounded-lg border border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-700 px-3 py-2 text-sm text-slate-500 dark:text-slate-400 cursor-not-allowed">
                    {profile.bloodType}
                    ...
                    {profile.allergies && profile.allergies.trim() ? (
                      profile.allergies
                        ...
                        .map((allergy, i) => (
                          <span
                            key={i}
                            className="px-2 py-1 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-sm rounded-md font-medium"
                          >
                            {allergy.trim()}
                          </span>
                        ))
