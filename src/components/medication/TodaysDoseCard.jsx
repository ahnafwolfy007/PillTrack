...
import { Check, X, Clock, SkipForward, Pill, ChevronDown, ChevronUp, Calendar, MapPin, AlertCircle, Loader2 } from 'lucide-react';
    ...
    const handleSkip = async () => {
        ...
        try {
            if (isVirtual) {
                ...
                await doseLogService.markAsSkipped(dose.id, skipReason || 'Skipped by user');
            }
        ...
        <div className="relative">
            <div className="flex items-center gap-2 flex-wrap">
                <Button 
                    ...
                    disabled={loading}
                >
                    {loading ? <Loader2 size={14} className="animate-spin" /> : <Check size={14} />} Take
                </Button>
                <Button 
                    size="sm" 
                    variant="outline"
            ...
            <AnimatePresence>
                {showSkipReason && (
                    <motion.div
                        ...
                        className="absolute top-full right-0 mt-2 bg-white rounded-lg shadow-lg border border-slate-200 p-3 z-50 min-w-[200px]"
                        onClick={(e) => e.stopPropagation()}
                    >
                        <p className="text-xs text-slate-500 mb-2">Reason for skipping:</p>
                            ...
                            {skipReasons.map((reason) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        skipReason === reason && "bg-primary/10 text-primary"
                                    )}
                                    onClick={(e) => {
                                        e.stopPropagation();
...
const TodaysDoseCard = ({ doses, medications = [], onUpdate, loading }) => {
    ...
    const allDoses = useMemo(() => {
        ...
        const activeMeds = (medications || []).filter(m => m.isActive === true || m.status === 'ACTIVE');
    ...
    const completedDoses = sortedDoses.filter(d => 
    ...
    const progress = totalDoses > 0 ? Math.round((completedCount / totalDoses) * 100) : 0;
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <div className="flex items-center gap-2 text-blue-100 text-sm mb-1">
                            <Calendar size={14} />
                            {formatDateInDhaka(new Date(), { weekday: 'long', month: 'short', day: 'numeric' })}
                        </div>
                ...
                {totalDoses > 0 && (
                    <div className="mt-4">
                        <div className="flex justify-between text-sm mb-1">
                            <span>{completedCount} of {totalDoses} doses completed</span>
                            <span>{progress}%</span>
                        </div>
                ...
                <div className="bg-blue-50 border-b border-blue-100 p-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-xs text-primary font-medium uppercase">Next Dose</p>
                                <h3 className="font-bold text-slate-800">
                                    {nextDose.medicationName || nextDose.medication?.name || 'Medication'}
                                </h3>
                                <p className="text-sm text-slate-500">
                                    {nextDose.dosage || nextDose.medication?.strength} • {formatDoseTime(nextDose.scheduledTime)}
                                    <span className="ml-2 text-primary font-medium">
                                        ({getRelativeTime(getTimeFromScheduled(nextDose.scheduledTime))})
                                    </span>
                                </p>
                            </div>
                        </div>
                        ...
                        <CardContent className="pt-4">
                            {sortedDoses.length === 0 ? (
                                <div className="text-center py-8 text-slate-500">
                                    ...
                                    <p className="text-sm text-slate-400 mt-1">Add medications to see your schedule</p>
                                </div>
                            ) : (
                                <div className="space-y-4">
                                    ...
                                    {pastPendingDoses.length > 0 && (
                                        <div>
                                            <h4 className="text-xs font-semibold text-red-500 uppercase mb-2 flex items-center gap-1">
                                                <AlertCircle size={12} />
                                                Overdue ({pastPendingDoses.length})
                                            </h4>
                                            <p className="text-xs text-slate-500 mb-2">
                                                Doses will be marked as missed 6 hours after scheduled time
                                            </p>
                                            <div className="space-y-2">
                                                {pastPendingDoses.map((dose, i) => {
                                                    ...
                                                    const minutesOverdue = currentTotalMinutes - scheduledMinutes;
                                                        ...
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <h5 className="font-semibold text-slate-800">
                                                                    {dose.medicationName || dose.medication?.name || 'Medication'}
                                                                </h5>
                                                                <p className="text-sm text-slate-500">
                                                                    {dose.dosage || dose.medication?.strength} • {formatDoseTime(dose.scheduledTime)}
                                                                    <span className="ml-1 text-red-500 text-xs">
                                                                        ({getRelativeTime(getTimeFromScheduled(dose.scheduledTime))})
                                                                    </span>
                                                                </p>
                                                                {isNearMissedCutoff && (
                                                                    <p className="text-xs text-red-600 font-medium mt-1">
                                                                        ⚠️ Will be marked as missed in {6 - hoursOverdue}h
                                                                    </p>
                                                                )}
                                                            </div>
                                                        </div>
                                                        <DoseAction dose={dose} onUpdate={onUpdate} isPastDose={true} />
                                                    </motion.div>
                                                    );
                                                })}
                                            </div>
                                    ...
                                    {upcomingDoses.length > 1 && (
                                        <div>
                                            <h4 className="text-xs font-semibold text-blue-500 uppercase mb-2 flex items-center gap-1">
                                                <Clock size={12} />
                                                ...
                                                {completedDoses.map((dose, i) => (
                                                    <motion.div
                                                        ...
                                                        className={cn(
                                                            ...
                                                            dose.status === 'TAKEN' && "bg-green-50 border border-green-100",
                                                            dose.status === 'SKIPPED' && "bg-amber-50 border border-amber-100",
                                                            dose.status === 'MISSED' && "bg-red-50 border border-red-100"
                                                        )}
                                                    >
                                                        <div className="flex items-center gap-3">
                                                            <div className={cn(
                                                                ...
                                                                dose.status === 'SKIPPED' && "bg-amber-100 text-amber-600",
                                                                dose.status === 'MISSED' && "bg-red-100 text-red-600"
                                                            )}>
                                                                {dose.status === 'TAKEN' && <Check size={18} />}
                                                                {dose.status === 'SKIPPED' && <SkipForward size={18} />}
