...
import { X, Pill, Clock, Bell, Package, Calendar, User, AlertTriangle, Edit, Loader2, Check, RefreshCw, TrendingDown, TrendingUp, Activity } from 'lucide-react';
import { Button } from '../ui/Button';
import { Card, CardContent } from '../ui/Card';
import { cn } from '../../utils/cn';
import { medicationService, doseLogService } from '../../services/api';

const MedicationDetailModal = ({ isOpen, onClose, medicationId, onEdit, onRefresh }) => {
    const [medication, setMedication] = useState(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState(null);
    const [recentDoses, setRecentDoses] = useState([]);
    const [adherenceStats, setAdherenceStats] = useState({ taken: 0, skipped: 0, missed: 0, total: 0 });
    ...
    const fetchMedicationDetails = async () => {
        ...
        try {
            ...
            try {
                const doseLogs = await doseLogService.getByMedication(medicationId, 0, 10);
                ...
                setAdherenceStats({ 
                    ...
                    percentage: (taken + skipped + missed) > 0 
    ...
    const getStockStatus = (currentQty, threshold) => {
        ...
        if (qty <= 0) return { label: 'Out of Stock', color: 'text-red-600 bg-red-50', icon: AlertTriangle };
        if (qty <= limit) return { label: 'Low Stock', color: 'text-amber-600 bg-amber-50', icon: TrendingDown };
        if (qty <= limit * 2) return { label: 'Moderate', color: 'text-blue-600 bg-blue-50', icon: Activity };
        return { label: 'Good Stock', color: 'text-green-600 bg-green-50', icon: TrendingUp };
    };
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h2 className="text-2xl font-bold">
                                            {loading ? 'Loading...' : medication?.name || 'Medication Details'}
                                        </h2>
                                        {medication && (
                                            <p className="text-blue-100">
                                                {medication.strength} • {medication.type || 'Medication'}
                                            </p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    ...
                                    <Card className="border-2 border-primary/20">
                                        <CardContent className="p-4">
                                            ...
                                            {(() => {
                                                const status = getStockStatus(medication.currentQuantity, medication.refillThreshold);
                                                ...
                                                const percentage = Math.round((currentQty / maxStock) * 100);
                                                const dailyDoses = parseInt(medication.frequency) || 1;
                                                const qtyPerDose = medication.quantityPerDose || 1;
                                                const daysRemaining = dailyDoses > 0 ? Math.floor(currentQty / (dailyDoses * qtyPerDose)) : 0;
                                                ...
                                                return (
                                                    <div className="space-y-4">
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <div className={cn("px-4 py-2 rounded-full font-medium", status.color)}>
                                                                {status.label}
                                                            </div>
                                                        </div>
                                                            ...
                                                            <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
                                                                <div 
                                                                    className={cn(
                                                                        ...
                                                                        currentQty <= threshold ? "bg-red-500" :
                                                                        currentQty <= threshold * 2 ? "bg-amber-500" : "bg-green-500"
                                                                    )}
                                                        ...
                                                        <div className="grid grid-cols-3 gap-4 pt-2">
                                                            <div className="text-center p-3 bg-slate-50 rounded-lg">
                                                                <p className="text-lg font-bold text-slate-800">{qtyPerDose}</p>
                                                                <p className="text-xs text-slate-500">Per Dose</p>
                                                            </div>
                                                            <div className="text-center p-3 bg-slate-50 rounded-lg">
                                                                <p className="text-lg font-bold text-slate-800">{dailyDoses * qtyPerDose}</p>
                                                        ...
                                                        {currentQty <= threshold && (
                                                            <div className="flex items-center gap-2 p-3 bg-amber-50 rounded-lg text-amber-700">
                                                                ...
                                                                <span className="text-sm font-medium">
                                                                    Refill recommended when stock falls below {threshold} units
                                                                </span>
                                                            </div>
                                                        )}
                                                    </div>
                                            ...
                                            <CardContent className="p-4">
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex justify-between">
                                                        ...
                                                        <span className="font-medium">{getFrequencyLabel(medication.frequency)}</span>
                                                    </div>
                                                    <div className="flex justify-between">
                                            ...
                                            <CardContent className="p-4">
                                                ...
                                                <div className="space-y-2">
                                                    <div className="flex justify-between">
                                                        ...
                                                        <span className={cn(
                                                            "font-medium",
                                                            medication.reminderTimes?.length > 0 ? "text-green-600" : "text-slate-400"
                                                        )}>
                                                            {medication.reminderTimes?.length > 0 ? 'Active' : 'Inactive'}
                                                        </span>
                                                    </div>
                                                    <div className="flex justify-between">
                                                        <span className="text-slate-500">Alert Before</span>
                                            ...
                                            <CardContent className="p-4">
                                                ...
                                                <div className="space-y-2">
                                                    ...
                                                    <div className="flex justify-between">
                                                        ...
                                                        <span className="font-medium">{medication.endDate ? formatDate(medication.endDate) : 'Ongoing'}</span>
                                                    </div>
                                                </div>
                                            </CardContent>
                                        </Card>
                                            ...
                                            <CardContent className="p-4">
                                                ...
                                                <p className="text-sm text-slate-500 mt-1">
                                                    Status: <span className={cn(
                                                        "font-medium",
                                                        medication.status === 'ACTIVE' ? "text-green-600" : "text-slate-400"
                                                    )}>
                                    ...
                                    {medication.instructions && (
                                        <Card>
                                            <CardContent className="p-4">
                                                ...
                                                <p className="text-slate-600">{medication.instructions}</p>
                                            </CardContent>
                                        </Card>
                                    )}
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <div className="flex-1">
                                                        <div className="flex justify-between text-sm mb-1">
                                                            ...
                                                            <span className="font-medium">{adherenceStats.percentage}%</span>
                                                        </div>
                                                    ...
                                                    <div className="flex gap-3 text-sm">
                                                        <div className="text-center">
                                                            <p className="text-green-600 font-bold">{adherenceStats.taken}</p>
                                                            <p className="text-xs text-slate-500">Taken</p>
                                                        </div>
                                                        <div className="text-center">
                                                            <p className="text-amber-600 font-bold">{adherenceStats.skipped}</p>
                                                            <p className="text-xs text-slate-500">Skipped</p>
                                                        </div>
                                                        <div className="text-center">
                                                            <p className="text-red-600 font-bold">{adherenceStats.missed}</p>
